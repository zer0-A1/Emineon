// Uses Neon pg client, replacing old Prisma-specific calls
...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      // If embeddings fail, fallback to lexical only
      vecLiteral = null;
    }

    // Run vector and lexical queries (best-effort)
    let vectorRows: Array<{ sourceId: string; vec_score: number }> = [];
    if (vecLiteral) {
      try {
        vectorRows = await query<any>(
          `SELECT "sourceId", 1 - (embedding_vec <=> $1::vector) AS vec_score
           FROM search_documents
           WHERE "sourceType" = 'CANDIDATE' AND embedding_vec IS NOT NULL
    ...
    // Normalize lexical scores to 0..1 based on max
    ...
    console.error('candidates-hybrid search failed', err);
